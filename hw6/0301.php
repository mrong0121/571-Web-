...
<div class="kane">
    ...
    <div class="kane2">
        <form id="myform" name="myform" action="" method="POST" onload="">
            ...
            <input type="text" name="zipcode" id="zipcode" placeholder="zip code" value="<?php echo isset($_POST["zipcode"])?$_POST["zipcode"]:""?>" disabled="" required>
...
if (isset($_POST["submitbut"])){
    ...
    if ($ready!=2 && $count==0){
        echo "<div class=\"error\"><b>No Records has been found<b></div>";
        $ready = 2;
    }
...
<script type="text/javascript">
    ...
    function createItemtable (){
        ...
        css.innerHTML += "#itemtable th{ text-align:center}";
        css.innerHTML += "#itemtable td{ border:2px solid lightgrey;}";
        document.body.appendChild(css);
        var tabletxt = "<table>";
        tabletxt +="<tr><th colspan=\"2\"><font size = '30px'><b>Item Details<b></th></tr>";
        for (i=0;i<itemarray.length;i++){
            if (Object.keys(itemarray[i]) == "Photo"){
                tabletxt +="<tr><td style='width:30%; padding-left:10px;'><b>"+Object.keys(itemarray[i])+"<b></td><td style='padding-left:10px;'>"+"<img src='"+itemarray[i][Object.keys(itemarray[i])]+"'width='" + "300px" + "' alt=''>"+"</td></tr>";
            }
            ...
            // }
                ...
                tabletxt += "<tr><td style='width=30%; padding-left:10px;'><b>"+Object.keys(itemarray[i])+"<b></td><td style='padding-left:10px;'>"+itemarray[i][Object.keys(itemarray[i])]+"</td></tr>";
            }
        }
        tabletxt+="</table>";
    ...
    function putArrow(){
        ...
        arrowtable = document.createElement("table");
        arrowtable.setAttribute("id", "arrowtable");
        document.getElementById("arrowdiv").appendChild(arrowtable);
        var tt = document.getElementById("arrowtable");
        var row = tt.insertRow(-1);
        var x = row.insertCell(-1);
        x.innerHTML="<div id='arrowupdiv'><font color='#d3d3d3'>click to show seller message</font></div>";
        var row = tt.insertRow(-1);
        var x = row.insertCell(-1);
        x.innerHTML="<img id='arrowupimage' src='"+arrowdown+"'>";
        var row = tt.insertRow(-1);
        var x = row.insertCell(-1);
        x.innerHTML="<div id='arrowdowndiv'><font color='#d3d3d3'>click to show similar items</font></div>";
        var row = tt.insertRow(-1);
        var x = row.insertCell(-1);
        x.innerHTML="<img id='arrowdownimage' src='"+arrowdown+"'>";
    ...
    function showSeller(){
        if (document.getElementById('arrowupimage').src==arrowdown){
            ...
            if (document.getElementById('similarerror')){
                var element=document.getElementById('similarerror');
                element.parentNode.removeChild(element);
                document.getElementById('arrowdownimage').setAttribute("src",arrowdown);
                document.getElementById('arrowdowndiv').innerHTML="<font color='#d3d3d3'>click to show seller message";
            }
            if (itemres.Item.Description){
                ...
                var x = row.insertCell(-1);
                x.innerHTML="<div id='sellerdiv'>"+seller+"</div>";
                document.getElementById('arrowupimage').setAttribute("src",arrowup);
                document.getElementById('arrowupdiv').innerHTML="<font color='#d3d3d3'>click to hide seller message";
            }else{
                var tt = document.getElementById("arrowtable");
                var row = tt.insertRow(2);
                var x = row.insertCell(-1);
                x.innerHTML="<div id='sellererror'><b>No Seller Message found<b></div>";
                document.getElementById('arrowupimage').setAttribute("src",arrowup);
                document.getElementById('arrowupdiv').innerHTML="<font color='#d3d3d3'>click to hide seller message";
            }

        } else{
            if(document.getElementById('sellererror')){
                var element=document.getElementById('sellererror');
                element.parentNode.removeChild(element);
                document.getElementById('arrowupimage').setAttribute("src",arrowdown);
                document.getElementById('arrowupdiv').innerHTML="<font color='#d3d3d3'>click to show seller message";
            }else{
                var element=document.getElementById('sellerdiv');
                element.parentNode.removeChild(element);
                document.getElementById('arrowupimage').setAttribute("src",arrowdown);
                document.getElementById('arrowupdiv').innerHTML="<font color='#d3d3d3'>click to show seller message";
            }
    ...
    function showSimilar(){
        if (document.getElementById('arrowdownimage').src==arrowdown){
            if (document.getElementById('sellerdiv')){
                var element=document.getElementById('sellerdiv');
                element.parentNode.removeChild(element);
                document.getElementById('arrowupimage').setAttribute("src",arrowdown);
                document.getElementById('arrowupdiv').innerHTML="<font color='#d3d3d3'>click to show seller message";
            ...
            if (simres.getSimilarItemsResponse.itemRecommendations.item){
                var tt = document.getElementById("arrowtable");
                var row = tt.insertRow(-1);
                var x = row.insertCell(-1);
                x.innerHTML="<div id='similardiv'></div>";
                ...
                document.getElementById('arrowdowndiv').innerHTML="<font color='#d3d3d3'>click to hide similar message";
            }else{
                var tt = document.getElementById("arrowtable");
                var row = tt.insertRow(-1);
                var x = row.insertCell(-1);
                x.innerHTML="<div id='similarerror'><b>No Similar Item found<b></div>";
                document.getElementById('arrowdownimage').setAttribute("src",arrowup);
                document.getElementById('arrowdowndiv').innerHTML="<font color='#d3d3d3'>click to hide similar message";
            }
        } else{
    ...
    function makeSimtabble(simItems){
        var simtable = document.createElement("table");
        simtable.setAttribute("id", "simtable");
        document.getElementById("similardiv").appendChild(simtable);
        var css = document.createElement("style");
        css.type = "text/css";
        ...
        document.body.appendChild(css);
        for (i=0;i<3;i++){
            var row = simtable.insertRow(i);
                for (j=0;j<simItems.length;j++){
                    var x = row.insertCell(-1);
                    if (i==0){
                        x.innerHTML += "<div style='width: 220px; padding-top: 20px;'><img src='" +simItems[j].imageURL + "'width='" + "150px" + "' alt=''></div>";
                    }
                    if (i==1){x.innerHTML ="<a href='#' onclick='clicksimName("+simItems[j].itemId+")'>"+simItems[j].title+"</a>";}
                    if (i==2){x.innerHTML = '$'+simItems[j].buyItNowPrice.__value__;}
            }
        }
    }
    ...
    function createTable(id){
        ...
        for (i=1;i<=count;i++) {
            ...
            x.innerHTML = '' + i;
            var x = row.insertCell(-1);
            x.innerHTML = "<img src='" + photo[i] + "'width='" + "50px" + "' alt=''>";
            var x = row.insertCell(-1);
